...
func (c *API) GetProfiles() (data ProfilesResponse, err error) {
	endpoint := zoho.Endpoint{
		Name:         "profiles",
		URL:          fmt.Sprintf("https://www.zohoapis.%s/crm/v2/settings/profiles", c.ZohoDomain),
		Method:       zoho.HTTPGet,
...
func (c *API) GetProfile(id string) (data ProfilesResponse, err error) {
	endpoint := zoho.Endpoint{
		Name:         "profiles",
		URL:          fmt.Sprintf("https://www.zohoapis.%s/crm/v2/settings/profiles/%s", c.ZohoDomain, id),
		Method:       zoho.HTTPGet,
		ResponseData: &ProfilesResponse{},
	}
